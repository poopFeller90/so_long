...
static int	ft_convert_sprites(t_vars *vars)
{
	...
	if (!vars->sprites->cobson)
		return (0);
	vars->sprites->wall = ft_helper(vars->mlx, 3, 64, 64);
	if (!vars->sprites->wall)
		return (0);
	vars->sprites->exit = ft_helper(vars->mlx, 4, 64, 64);
	if (!vars->sprites->exit)
		return (0);
	vars->sprites->ground = ft_helper(vars->mlx, 5, 64, 64);
	if (!vars->sprites->ground)
		return (0);
	if (!ft_helper_soylent(vars, 64, 64))
		return (0);
	return (vars->sprites->converted = 1, 1);
}
...
static void	ft_draw_sprite(t_sprites *sprites, void *mlx, void *win, char c)
{
	if (c == '0')
		...
		mlx_put_image_to_window(mlx, win,
			sprites->soylent[sprites->i_frame], sprites->x, sprites->y);
	else if (c == 'P')
		mlx_put_image_to_window(mlx, win,
			sprites->soyjak, sprites->x, sprites->y);
	else if (c == 'H')
		mlx_put_image_to_window(mlx, win,
			sprites->cobson, sprites->x, sprites->y);
	else if (c == 'E')
		mlx_put_image_to_window(mlx, win,
